...
<body class="bg-gray-100">
    ...
    <!-- Sidebar -->
    <div id="residentSidebar"
        class="w-full md:w-64 bg-yellow-500 text-gray-900 h-screen flex flex-col fixed inset-y-0 left-0 z-40 transform md:transform-none transition-transform duration-200 -translate-x-full md:translate-x-0 overflow-y-auto">
        <div class="p-6 flex flex-col items-center border-b border-yellow-400">
            ...
            <div class="flex justify-center mb-2 -mt-2">
                <div class="w-24 h-24 rounded-full overflow-hidden shadow-xl">
                    <img src="{{ asset('LOGO.png') }}" alt="Barangay Logo" class="w-full h-full object-cover scale-125">
                </div>
            </div>
            <!-- Logo Name -->
        ...
        <nav class="flex-1 mt-4">
            ...
            <a href="{{ route('resident.settings.index') }}"
                class="flex items-center py-3 px-6 transition duration-200 hover:bg-yellow-400 {{ request()->routeIs('resident.settings.index') ? 'bg-yellow-400' : '' }}">
        ...
        <div class="mt-auto p-4 border-t border-yellow-400">
            <form method="POST" action="{{ route('logout') }}">
                @csrf
                <button type="submit"
                    class="flex items-center w-full py-3 px-6 hover:bg-yellow-400 transition duration-200 rounded">
    ...
    <div class="md:ml-64 p-4 md:p-8 min-h-screen">
        ...
        <div class="flex items-center justify-between mb-4 md:mb-8">
            <div class="flex items-center gap-3">
                <button id="btnResidentMenu"
                    class="md:hidden inline-flex items-center justify-center w-10 h-10 rounded bg-yellow-500 text-white"><i
            ...
            <div class="flex items-center space-x-3 md:space-x-4">
                ...
                <div class="w-9 h-9 md:w-10 md:h-10 bg-yellow-500 rounded-full flex items-center justify-center">
                    <span
                        class="text-white font-bold">{{ strtoupper(substr(Auth::user()->first_name ?? 'R', 0, 1)) }}</span>
                </div>
            </div>
        </div>
