                ...
                <tr class="hover:bg-gray-50">
                    ...
                    <td class="px-6 py-4 text-center">
                        <button type="button" 
                                ...
                                data-resident="{{ $res->user->fullname }}"
                                data-service="{{ $res->service->name }}"
                                data-date="{{ $res->reservation_date->format('Y-m-d') }}" 
                                data-start="{{ \Carbon\Carbon::parse($res->start_time)->format('g:i A') }}" 
                                data-end="{{ \Carbon\Carbon::parse($res->end_time)->format('g:i A') }}" 
                                data-in="{{ $res->actual_time_in ? \Carbon\Carbon::parse($res->actual_time_in)->format('g:i A') : '' }}" 
                                data-out="{{ $res->actual_time_out ? \Carbon\Carbon::parse($res->actual_time_out)->format('g:i A') : '' }}"
        ...
        <div class="p-6 space-y-4">
            ...
            <form id="timesForm" method="POST">
                ...
                <div class="space-y-4">
                    ...
                    <!-- Action Buttons -->
                    <div id="actionsRow" class="flex justify-between gap-2">
                        <div>
                            <button type="button" id="btnCancelReservation" class="px-4 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700">
                                Cancel Reservation
                            </button>
                        </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.querySelectorAll('.btn-view').forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            if (btnCancelReservation) {
                if (isLocked) {
                    btnCancelReservation.classList.add('hidden');
                } else {
                    btnCancelReservation.classList.remove('hidden');
                    btnCancelReservation.dataset.reservationId = id;
                }
            }
    ...
    if (btnCancelReservation) {
        btnCancelReservation.addEventListener('click', function() {
            ...
            const resident = document.getElementById('m_resident').textContent;
            const ref = document.getElementById('m_ref').textContent;
            
            if (confirm(`Are you sure you want to cancel this reservation?\n\nReference: ${ref}\nResident: ${resident}\n\nThis action cannot be undone.`)) {
                // Create form to submit cancel request
                const cancelForm = document.createElement('form');
                cancelForm.method = 'POST';
                cancelForm.action = `{{ url('admin/reservations') }}/${reservationId}/cancel`;
                ...
                cancelForm.appendChild(methodInput);
